 ...
 * File: StaffDashboardController.java
...
public class StaffDashboardController {
    ...
    private Map<String, Object> roomResponse(Camera room) {
        ...
        response.put("numero", displayRoomNumber(room.getIdCamera()));
    ...
    /* BLOCCO DIDATTICO — Converte l'id DB in numero camera mostrato allo staff. */
    private int displayRoomNumber(Long id) {
        if (id == null || id <= 0) {
            return 0;
        }
        int value = id.intValue();
        int floor = ((value - 1) / 10) + 1;
        int position = ((value - 1) % 10) + 1;
