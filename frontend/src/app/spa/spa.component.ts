 ...
 * ---------------------------------------------------------------------------
 * Pagina SPA: categorie, carosello, dettaglio trattamento e modale di prenotazione.
...
export class SpaComponent implements OnDestroy {
  ...
  /** BLOCCO DIDATTICO — Passa tra percorso SPA persone e percorso pet. */
  switchCategory(cat: 'human' | 'pet'): void {
    this.currentCategory = cat;
    this.currentIndex = 0;
  }

  /** BLOCCO DIDATTICO — Seleziona manualmente una slide del carosello. */
  selectSlide(index: number): void {
    this.currentIndex = index;
  }

  /** BLOCCO DIDATTICO — Sposta il carosello avanti o indietro. */
  ...
  /** BLOCCO DIDATTICO — Avvia lo scorrimento automatico usato come tour visivo. */
  startVideoTour(): void {
  ...
  /** BLOCCO DIDATTICO — Ferma il timer del tour automatico. */
  stopVideoTour(): void {
  ...
  /** BLOCCO DIDATTICO — Apre la modale di prenotazione SPA. */
  openBookingModal(): void {
    this.isModalOpen = true;
    this.isConfirmed = false;
  }

  /** BLOCCO DIDATTICO — Chiude la modale senza inviare il form. */
  closeModal(): void {
    this.isModalOpen = false;
  }

  /** BLOCCO DIDATTICO — Valida il form SPA e mostra la conferma prevista dal prototipo. */
  ...
  /** BLOCCO DIDATTICO — Pulisce i timer quando Angular distrugge il componente. */
  ngOnDestroy(): void {
    if (this.videoInterval) {
      clearInterval(this.videoInterval);
    }
