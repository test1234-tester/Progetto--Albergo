 ...
 * ---------------------------------------------------------------------------
 * Flusso principale camere: ricerca, filtri, selezione, servizi, prenotazione e caparra.
...
export class RoomsComponent implements OnInit {
  ...
  /** BLOCCO DIDATTICO — Ciclo di vita: carica le camere appena il componente viene creato. */
  ngOnInit(): void {
  ...
  /** BLOCCO DIDATTICO — Applica date e numero ospiti alla ricerca e aggiorna le camere mostrate. */
  onSearch(): void {
  ...
  /** BLOCCO DIDATTICO — Porta l’utente dal risultato sintetico al dettaglio della camera scelta. */
  ...
  /** BLOCCO DIDATTICO — Torna all’elenco camere azzerando la selezione corrente. */
  deselezionaStanza(): void {
    this.selectedRoom.set(null);
    this.errorMessage.set(null);
  }

  /** BLOCCO DIDATTICO — Aggiunge o rimuove un servizio opzionale dal preventivo. */
  ...
  /** BLOCCO DIDATTICO — Alterna visualizzazione/nascondimento del CVC senza modificarne il valore. */
  toggleMostraCvc(): void {
    this.mostraCvc.update((value) => !value);
  }

  /** BLOCCO DIDATTICO — Valida il flusso e invia la prenotazione al backend. */
  confermaEPay(): void {
    const room = this.selectedRoom();
    if (!room) return;
  ...
  /** BLOCCO DIDATTICO — Azzera lo stato della conferma per permettere una nuova ricerca. */
  ...
  /** BLOCCO DIDATTICO — Fallback: nasconde/gestisce un’immagine che non può essere caricata. */
  ...
  private toRoomCard(room: {
    ...
    const tipo = this.roomType(id);
    const capienza = room.occupanti && room.occupanti > 0 ? room.occupanti : (id % 4) + 2;

    return {
      idCamera: id,
      nome: room.nome?.trim() || `${tipo} ${this.displayRoomNumber(id)}`,
  ...
  /** BLOCCO DIDATTICO — Numero commerciale mostrato al cliente (101, 102, 201...). */
  private displayRoomNumber(id: number): number {
    const floor = Math.floor((id - 1) / 10) + 1;
    const position = ((id - 1) % 10) + 1;
    return floor * 100 + position;
  }

  /** BLOCCO DIDATTICO — Nome leggibile della categoria camera usato come fallback. */
  private roomType(id: number): string {
    const position = ((id - 1) % 10) + 1;
    const types = [
      'Camera Comfort',
      'Camera Armonia',
  ...
  /** BLOCCO DIDATTICO — Calcola il numero totale di persone richieste. */
  ...
  /** BLOCCO DIDATTICO — Utility usata dal template per mostrare un campo in errore solo al momento giusto. */
